...
// ...
pthread_barrier_t barrierTriangles;
pthread_barrier_t barrierFragments;
...
static void *triangle_stage(void *a)
{
  ...
  while (triangle_creator_get_next_triangle(triangle_creator, &triangle))
  {
    ...
    if (triangle_is_sync(&triangle))
    {
      ...
      pthread_barrier_wait(&barrierTriangles);
      // 
      ...
      // Utiliser ici une barrière
      pthread_barrier_wait(&barrierFragments);
...
void *raster_stage(void *a)
{
  while (1)
  {
...
#endif
    {
      ...
      {
        // Production des fragments dans le tableau des fragments :
        // remplacer les ...
        sem_wait(&fragment_sem_prod);
          pthread_mutex_lock(&fragment_mutex);
...
int main(int argc, char *argv[])
{
  ...
  // ...
  pthread_barrier_init(&barrierTriangles, NULL, nb_rasters+1);
  pthread_barrier_init(&barrierFragments, NULL, nb_shaders+1);
  pthread_barrier_init(&barrierAll, NULL, nb_rasters+nb_shaders+1);
